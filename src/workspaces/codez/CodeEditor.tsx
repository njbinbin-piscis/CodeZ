...
function tabCompleteEnabled(): boolean {
  return localStorage.getItem("agentz-tab-complete") !== "0";
}

function completionModelId(): string | null {
  const id = localStorage.getItem("agentz-completion-model-id");
...
export default function CodeEditor({ tab, projectDir, onChange, onSave, reveal }: CodeEditorProps) {
  ...
  const applyInlinePreview = useCallback((s: InlineEditState, proposed: string) => {
    ...
    editor.executeEdits("agentz-inline-edit", [{ range: s.range, text: proposed }]);
    ...
    const addedDecos = ops
      ...
      .map((o) => ({
        ...
        options: { isWholeLine: true, className: "agentz-inline-added-line" },
      }));
    ...
    if (removed.length > 0) {
      ...
      dom.className = "agentz-inline-removed-zone";
  ...
  return (
    <div className="agentz-editor-wrap">
      ...
      {inline && (
        <div className="agentz-inline-edit">
          {inline.applied ? (
            <div className="agentz-inline-edit-bar">
              <span className="agentz-inline-edit-hint">Review the inline diff:</span>
              <button className="agentz-inline-accept" onClick={acceptInline}>
                Accept ⏎
              </button>
              <button className="agentz-inline-edit-cancel" onClick={rejectInline}>
                Reject ⎋
              </button>
            </div>
          ) : (
            <div className="agentz-inline-edit-bar">
              <input
                autoFocus
                className="agentz-inline-edit-input"
              ...
              <button className="agentz-inline-edit-cancel" onClick={cancelInline}>
                Cancel
              </button>
            </div>
          )}
          {inline.error && <div className="agentz-inline-edit-error">{inline.error}</div>}
        </div>
      )}
    </div>
  );
