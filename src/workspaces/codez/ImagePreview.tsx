...
export default function ImagePreview({ src, name, size }: ImagePreviewProps) {
  ...
  return (
    <div className="agentz-image-preview" tabIndex={0}>
      <div className="agentz-image-preview-meta">
        <span>{name}</span>
        {size != null && size > 0 && (
          <span className="agentz-image-preview-size">
            {(size / 1024).toFixed(size > 1024 * 1024 ? 1 : 0)}
            {size > 1024 * 1024 ? " MB" : " KB"}
          </span>
        )}
      </div>
      <div className="agentz-image-preview-frame">
