  ...
  return (
    <div className="agentz-settings-tabpanel">
      <section className="agentz-settings-section">
        ...
        {items.length === 0 ? (
          <div className="agentz-wb-empty">尚未安装任何连接器</div>
        ) : (
          <div className="agentz-wb-list">
            {items.map((c) => (
              <div key={c.id} className="agentz-wb-row" style={{ flexDirection: "column", alignItems: "stretch", gap: 8 }}>
                ...
                {editing === c.id && (
                  ...
                  >
                    {c.fields.length === 0 && (
                      <span className="agentz-wb-meta">该连接器未声明凭证字段</span>
                    )}
                    {c.fields.map((f) => (
                      <input
                        key={f.key}
