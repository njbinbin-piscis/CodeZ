...
function TestNode({ item, controllerId, depth }: { item: TestItemDto; controllerId: string; depth: number }) {
  return (
    <div>
      <div className="agentz-tree-row" style={{ paddingLeft: 8 + depth * 14 }}>
        <button
          className="agentz-test-run"
          title="Run test"
    ...
    <div className={`ide-bottom-panel${open ? "" : " is-hidden"}`} style={open ? { height } : undefined} aria-hidden={!open}>
      ...
      <div className="ide-bottom-body">
        ...
        {current === "output" && (
          <div className="ide-bottom-pane scroll">
            {activeOutput ? (
              ...
              <div className="agentz-ext-panel-empty">{t("extensions.noOutput") || "No output channels."}</div>
            )}
          </div>
        )}
        ...
        {current === "views" && (
          <div className="ide-bottom-pane scroll">
            {ui.treeViews.length === 0 ? (
              <div className="agentz-ext-panel-empty">{t("extensions.noViews") || "No views contributed."}</div>
            ) : (
              ui.treeViews.map((v) => (
                <div key={v.viewId} className="agentz-ext-view-section">
                  <div className="agentz-ext-view-title">{v.viewId}</div>
        ...
        {current === "scm" && (
          <div className="ide-bottom-pane scroll">
            {ui.scm.length === 0 ? (
              <div className="agentz-ext-panel-empty">{t("extensions.noScm") || "No source control providers."}</div>
            ) : (
              ui.scm.map((p) => (
                <div key={p.handle} className="agentz-ext-view-section">
                  <div className="agentz-ext-view-title">{p.label}</div>
                  {p.groups.map((g) => (
                    <div key={g.handle} className="agentz-scm-group">
                      <div className="agentz-scm-group-label">
                        {g.label} <span className="agentz-ext-panel-count">{g.resources.length}</span>
                      </div>
                      {g.resources.map((r) => (
                        <div key={r.handle} className="agentz-scm-resource" title={r.tooltip}>
        ...
        {current === "tests" && (
          <div className="ide-bottom-pane scroll">
            {ui.tests.length === 0 ? (
              <div className="agentz-ext-panel-empty">{t("extensions.noTests") || "No test controllers."}</div>
            ) : (
              ui.tests.map((c) => (
                <div key={c.controllerId} className="agentz-ext-view-section">
                  <div className="agentz-ext-view-title">{c.label}</div>
                  {c.items.map((item) => (
        ...
        {current === "webviews" && (
          <div className="ide-bottom-pane scroll">
            {ui.webviews.length === 0 ? (
              <div className="agentz-ext-panel-empty">{t("extensions.noWebviews") || "No webviews open."}</div>
            ) : (
              ui.webviews.map((wv) => (
                <div key={wv.handle} className="agentz-ext-webview-section">
                  <div className="agentz-ext-view-title">{wv.title}</div>
