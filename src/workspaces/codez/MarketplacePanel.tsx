...
export default function MarketplacePanel({ onClose, initialCategory = "skill" }: MarketplacePanelProps) {
  ...
  return (
    <div className="agentz-market-overlay" onClick={onClose}>
      <div className="agentz-market" onClick={(e) => e.stopPropagation()}>
        <div className="agentz-market-head">
          <h2>{t("market.title")}</h2>
          <button type="button" className="agentz-market-close" onClick={onClose} title={t("common.close")}>
            ✕
          </button>
        </div>

        <div className="agentz-market-cats" role="tablist">
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
              type="button"
              ...
              className={`agentz-market-cat ${category === cat ? "active" : ""}`}
        ...
        <div className="agentz-market-grid">
          {loading && items.length === 0 ? (
            <div className="agentz-market-empty">{t("market.loading")}</div>
          ) : items.length === 0 ? (
            <div className="agentz-market-empty">{t("market.empty")}</div>
          ) : (
            items.map((item) => (
