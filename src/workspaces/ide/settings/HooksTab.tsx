...
/** Settings tab: manage Cursor-style hooks in `.codez/hooks.json`. */
  ...
  return (
    <div className="codez-settings-tabpanel">
      <section className="codez-settings-section">
        <h3>{t("hooks.title")}</h3>
        <p className="codez-settings-hint">{t("hooks.hint")}</p>
        ...
        {loading ? (
          ...
          <>
            {config.hooks.length === 0 && <div className="codez-wb-empty">{t("hooks.empty")}</div>}
            {config.hooks.map((h, idx) => (
              <div key={h.id} className="codez-wb-hook">
                <div className="codez-wb-hook-head">
                  <input
                ...
                <div className="codez-wb-hook-actions">
                  <button
                    type="button"
                ...
                {runResult[h.id] && (
                  <pre className="codez-wb-hook-output">
            ...
            <div className="codez-wb-hook-footer">
              <button type="button" className="codez-llm-add" onClick={add}>
                + {t("hooks.add")}
              </button>
              <button
                type="button"
