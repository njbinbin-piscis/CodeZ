# AgentZ — 类 Cursor AI IDE 设计文档

> AgentZ 是一个类 Cursor 的 **AI IDE**，对外提供两种一等模式：
> **IDE 模式**（编辑器为中心，≈ Cursor）与 **Agent 模式**（任务为中心，≈ Codex）。
> 二者共享 [`piscis-engine`](https://github.com/njbinbin-piscis/piscis-engine) 的
> agent 内核（`piscis-core` + `piscis-kernel`），以 git 依赖方式引入。
>
> **本文档描述的是 AgentZ 仓库的真实实现**，而非愿景。所有"已完成 / 计划中"以
> §9 里程碑表为准。
...
AgentZ 是一个**独立仓库**，只把 `piscis-engine` 当作**共享 agent 内核**消费：

- 复用内核的：agent loop（`piscis_kernel::agent`）、多模型 LLM 客户端、
  中立工具集（`piscis_kernel::tools` 的 file/shell/code_run/search/plan…）、
  策略门（`PolicyGate`）、compaction v2（rolling summary + state frame）、
  headless turn runner（`piscis_kernel::headless`）。
- AgentZ 自己实现的（host 层）：双模式前端、IDE 工作区（Monaco/LSP/终端/Git/watcher）、
...
看板、`@mention` 调度、子进程 subagent runtime）。**AgentZ 有意不接它**——在 host 装配
工具注册表时显式关闭：
...
（需要看板 UI、命名 agent 生命周期、心跳/看门狗/对账协议）。AgentZ 选择更轻的两件套：
...
二者复用同一 agent loop，仅 **工具暴露面 + 上下文 + 产物形态**不同。AgentZ 用
...
```
AgentZ/
├── Cargo.toml                  # Rust workspace；piscis-core / piscis-kernel 走 git 依赖
├── crates/agentz-host/          # kernel-link smoke 二进制
...
内核复用的关键路径：`commands/chat_turn.rs::run_agentz_turn` 在 AgentZ 侧装配工具注册表、
...
- **Chat 侧栏 + @引用 + Apply**：`AssistantPanel` 驱动 `chat_send` → `run_agentz_turn`
...
AgentZ 的多 agent 能力就是 **`delegate` 只读研究子 agent**，而不是 Pool/Koi。
...
**边界（与 Cursor 的差异）**：AgentZ 目前**没有**可并发的"写型"子 agent，也没有持久命名
...
提交目标(prompt + 项目 + 可选基准分支)
   │
   ├─ 1. 隔离：git worktree 新建 workz/task-<id> 分支
   │        （worktree 落在 <project>/../.agentz-worktrees/task-<id>）
   ├─ 2. 自治：跑 run_agentz_turn（agent 模式全量工具）— 计划→编辑→跑工具→迭代
...
并行隔离。**这是 AgentZ 的"多 agent 并行"落点**（任务级并行，而非单任务内多 agent）。
...
- 实现：`commands/codebase.rs` 把源文件按 **50 行窗口**分块，存进
  `{project}/.agentz/index.db`（SQLite）。
- 检索：**关键词 / 词频排序 + 路径名加权**（非向量嵌入），不需要 embedding API key。
  schema 故意留简单，后续可加一列 embedding，复用 `piscis_kernel::memory::vector`。
...
- **ClawHub 技能市场**：`clawhub_search` / `clawhub_install`；安装到 `{config}/skills/installed/`。
  turn 启动时按"渐进式披露"注入 `## Available skills`（仅列 name + 摘要，命中任务时让
  agent 先 `file_read` 完整 `SKILL.md`）。可执行型技能落在 `{config}/user-tools/`。
- **技能进化（v0.5.0）**：四象限存储（`installed/`、`.draft/`、`learned/`、`.archive/`）；
  `skill_manage` 受控写入；回合后 background review + 记忆提取；Curator 空闲维护；
  全局 `{config}/piscis.db` 存 skill 元数据/revisions（与会话 DB 分离）。
- **项目规则**：`{workspace}/.agentz/rules/`（优先）或 `.cursor/rules/`（兼容）下的
...
| **M5** | 代码库索引(`.agentz/index.db`，关键词排序) + `codebase_search` + `@codebase` + Tab 补全 | ✅ 已完成 |
...
- headless / 云端长时任务 runner 与沙箱化（内核已具 `piscis-cli`/NDJSON 基建可铺路）。

---

## 10. VS Code `.vsix` 兼容（M2.5）

> 核心事实：AgentZ 用的是 **Monaco**（编辑器内核），不是完整 VS Code 工作台。
...
3. **多 agent 协作**：WorkZ 提供两条路径 — **蜂群（swarm）** 由协调者 turn + `pool_org`/`pool_chat` 驱动，wave 顺序由 `org_spec` + `depends_on` + `workflow_hint` 约束，patrol 恢复 stale/pending，heartbeat 在有 open todo 时发起轻量协调者 turn；**工作流（workflow）** 由确定性 graph driver 逐步执行，每次 run 使用独立 pool 以免 transcript 混杂。并发仍通过只读 `delegate` 与多 worktree 任务隔离。
...
## 12. WorkZ 协作机制（蜂群 vs 工作流）

| 模式 | 驱动 | 收敛 |
|---|---|---|
| **蜂群 swarm** | 协调者 LLM turn + `pool_org` / `pool_chat` | 协调者 system 协议（`get_todos` → 处理 blocked/needs_review → `post_status`）；成员 todo turn 使用 kernel **Stop Gate** 六层 prompt；patrol 恢复 stale；heartbeat 在有 attention 时补一轮协调 |
| **工作流 workflow** | `runtime/workflow` 图驱动 | 步数/loop 上限 + driver 互斥；LLM 分支无匹配且无 `default_to` 则 fail |
