...
//! M5 codebase index (`{project}/.codez/index.db`). The generation is
...
//! offline without an LLM. The result is written to `{project}/.codez/REPO_WIKI.md`
...
pub fn generate(root: &Path) -> Result<String, String> {
    ...
    md.push_str(&format!(
        "_Auto-generated by CodeZ from the codebase index on {now}._\n\n"
    ));
    md.push_str(&format!(
...
/// Generate the Repo Wiki, write it to `.codez/REPO_WIKI.md`, and return the
...
pub async fn repo_wiki_generate(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let out_dir = root.join(".codez");
        std::fs::create_dir_all(&out_dir).map_err(|e| format!("create .codez: {e}"))?;
        let out = out_dir.join("REPO_WIKI.md");
        std::fs::write(&out, &md).map_err(|e| format!("write wiki: {e}"))?;
        Ok(RepoWikiResult {
            path: ".codez/REPO_WIKI.md".to_string(),
            markdown: md,
        })
    })
    .await
