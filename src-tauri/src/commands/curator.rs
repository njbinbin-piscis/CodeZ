...
pub fn rollback_latest_backup(ctx: &SkillEvolutionCtx) -> Result<(), String> {
...
fn is_writable_curator_target(root: &Path, db: &Database, skill_id: &str) -> bool {
...
fn list_curator_candidates(root: &Path, db: &Database) -> Vec<DraftCatalogEntry> {
    let mut out = Vec::new();
...
pub async fn run_curator_pass(
    ...
    let stale = mark_stale_skills(ctx, &cfg, dry_run).await?;
    let archived = archive_stale_skills(ctx, &cfg, dry_run).await?;
    let (merged, patched) = llm_merge_duplicates(ctx, &cfg, dry_run).await?;
...
pub async fn maybe_run_curator_idle(app: &AppHandle) {
    let Ok(ctx) = SkillEvolutionCtx::open(app) else {
        return;
    };
    let cfg = {
        let s = ctx.settings.lock().await;
    ...
    let _ = run_curator_pass(app, &ctx, false).await;
}
