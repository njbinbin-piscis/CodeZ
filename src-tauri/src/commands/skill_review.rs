...
fn summarize_turn(messages: &[LlmMessage], max_chars: usize) -> String {
...
pub async fn run_background_skill_review(
    global: SkillEvolutionCtx,
    project_db: Arc<tokio::sync::Mutex<piscis_kernel::store::db::Database>>,
    session_id: String,
    ...
    let evo: SkillEvolutionSettings = {
        let s = global.settings.lock().await;
...
"#,
    ...
    let root = global.skills_root();
    let session_ref = session_id.as_str();

    {
        let pdb = project_db.lock().await;
        for mem in decision.save_memories.iter().take(2) {
            ...
            let _ = pdb.save_memory_structured(
                &mem.content,
